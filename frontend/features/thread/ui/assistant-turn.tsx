...
function AnswerProse({
  ...
  const parts = text.split(/(\[[SFVCXH]\d+\])/g)
  return (
    <p className="whitespace-pre-wrap text-answer leading-[var(--lh-answer)] text-foreground">
      {parts.map((p, i) => {
        ...
        const id = m[1]
        if (!known.has(id)) {
          // A citation that leads nowhere is a finding about the answer, not
          // a link. Rendering it as an ordinary chip lends the sentence the
          // appearance of support the verifier just refused it.
          return (
            <span
              key={i}
