...
interface FloatingTelemetryHUDProps {
  ...
  modelName?: string
  taskRunning?: boolean
  /** Tokens per second, measured. Omitted until a run reports one. */
  tokensPerSecond?: number | null
  /** The audit chain head, from /api/audit/chain. */
...
export function FloatingTelemetryHUD({
  ...
  // No default model name. The registry holds qwen3:8b and qwen2.5:3b, so a
  // hardcoded 'Qwen-2.5-72B-Local' would name a model this host cannot run.
  modelName,
  taskRunning = false,
  ...
  // Both of these are reported by the caller or not shown at all. They used
  // to be simulated — a random 44-62 tok/s and a SHA-256 head whose leading
  // bytes were re-rolled every 600ms — which is a picture of telemetry rather
  // than telemetry.
  const tokenSpeed = tokensPerSecond
  const hashHead = chainHead
        ...
        <div className="flex flex-col gap-3 p-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex flex-col gap-0.5 rounded border border-border bg-surface-sunken p-2">
              ...
              <span className="font-mono text-[13px] font-semibold text-foreground">
                {taskRunning && tokenSpeed != null ? `${tokenSpeed} tok/s` : taskRunning ? 'RUNNING' : 'IDLE · READY'}
              </span>
          ...
          <div className="flex flex-col gap-1 rounded border border-border bg-ink p-2 text-ink-foreground">
            ...
            <div className="overflow-hidden font-mono text-[10px] text-[var(--sovereign)] tracking-tight truncate">
              {hashHead ?? 'chain head unavailable'}
            </div>
          </div>
