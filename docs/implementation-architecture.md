...
|---|---|---|
| PostgreSQL + Redis | **SQLite** (stdlib `sqlite3`) for users, tasks, files, knowledge documents and audit metadata; in-process `asyncio.Queue` for the job queue | No multi-worker coordination needed for a single-machine demo. Deferred: Postgres/Redis, horizontal worker scaling. |
| pgvector + reranker | Local Ollama embeddings persisted in SQLite and cosine-searched in-process; deterministic BM25 retrieval remains available when the embedding model is not installed | Same retrieval *behavior* (embed, search, cite), tiny corpus. Deferred: reranker, pgvector, large-scale ingestion. |
...
| Container platform | Docker Compose: `frontend`, `api`, and `model-server` (Ollama) on an internal-only network; the process sandbox remains inside the API container | Proves the containerization concept without Kubernetes. Deferred: full orchestration, autoscaling and a separately scheduled sandbox service. |
